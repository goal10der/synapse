...
export default function AudioPage() {
    ...
    const setMaxVolume = (decimal: number) => {
        ...
        if (updateKeybindsLimit(decimal)) {
            ...
            execAsyncNoWait(`wpctl set-volume @DEFAULT_AUDIO_SINK@ ${speaker.volume} --limit ${decimal}`);
        }
    };
    ...
    return (
        ...
        >
                    ...
                    <Gtk.Button
                        $={(self: any) => {
                            const updateMuteIcon = () => {
                                const icon = speaker.mute ? "audio-volume-muted-symbolic" : "audio-volume-high-symbolic";
            ...
            <Gtk.Box orientation={Gtk.Orientation.VERTICAL} cssClasses={["settings-card"]} spacing={16}>
                ...
                <Gtk.Box orientation={Gtk.Orientation.HORIZONTAL} spacing={12} halign={Gtk.Align.START}>
                    <Gtk.Entry
                        widthRequest={100}
                        placeholderText="100"
                        $={(self: any) => {
                    ...
                    />
                    <Gtk.Label label="%" cssClasses={["dim-label"]} valign={Gtk.Align.CENTER} />
                    <Gtk.Button
                        label="Apply"
                        onClicked={applyMaxVolume}
                    />
                </Gtk.Box>
                        ...
                        <Gtk.Button
                            ...
                            $={(self: any) => {
                                maxVolume.subscribe(max => {
                                    ...
                                    const classes = currentPercent === percent 
            ...
            <Gtk.Box orientation={Gtk.Orientation.VERTICAL} cssClasses={["settings-card"]} spacing={16}>
                ...
                <Gtk.Scale
                    ...
                    digits={0}
                    $={(self: any) => {
                        // Set initial range to current max
                        self.set_range(0, maxVolume.get());
                        self.set_increments(0.01, 0.05);
                        
                        // Update value when speaker volume changes
                        const updateValue = () => {
                            self.set_value(speaker.volume);
                        };
                        speaker.connect("notify::volume", updateValue);
                        updateValue();

                        // Update range when max volume changes
                        maxVolume.subscribe(max => {
                            self.set_range(0, max);
                        });

                        // Set volume when slider changes, with limit
                        self.connect("value-changed", () => {
                            const val = self.get_value();
                            execAsyncNoWait(`wpctl set-volume @DEFAULT_AUDIO_SINK@ ${val} --limit ${maxVolume.get()}`);
                        });
                    }}
                />
            </Gtk.Box>
                    ...
                    {[0, 25, 50, 75, 100].map(percent => (
                        <Gtk.Button
                            ...
                            onClicked={() => {
                                ...
                                execAsyncNoWait(`wpctl set-volume @DEFAULT_AUDIO_SINK@ ${decimal} --limit ${maxVolume.get()}`);
                            }}
                            cssClasses={["volume-preset-btn"]}
                            hexpand
                        />
                    ))}
                </Gtk.Box>
            ...
            <Gtk.Box orientation={Gtk.Orientation.VERTICAL} cssClasses={["settings-card"]} spacing={10}>
                ...
                <Gtk.Label 
                    $={(self: any) => {
                        const updateDevice = () => {
                            self.set_label(`Active Output: ${speaker.description || "Unknown Device"}`);
                        };
